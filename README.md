...
The HSU Early Warning System is an end-to-end analytics and dashboard solution designed to help Horizon State University proactively identify and support students at risk of dropping out. The project consolidates academic performance, LMS engagement, financial stress, and wellness indicators into a unified data model, applies a Random Forest classifier to generate student-level risk scores, and exposes the results through a multi-role Streamlit web application.

Advisors and administrators can see cohort-level risk patterns, drill into individual students, and log interventions, while students themselves can log in to a dedicated portal to view their own GPA, progress, finances, and alerts. The system demonstrates how a combination of data engineering, machine learning, and user-centered dashboards can move the institution from reactive crisis management to proactive, data-driven support for student success.

## Problem Context & Stakeholders

Many universities, including the fictional Horizon State University (HSU), struggle with fragmented student data and reactive retention practices. Risk signals are scattered across systems—grades, LMS usage, payment status, and counseling notes—so advisors often discover problems only after students fail courses or withdraw.

Key stakeholders for this project include:

- **Students** who may be quietly struggling with academics, engagement, finances, or wellness.
- **Academic advisors** who need a clear, prioritized view of which students require outreach.
- **Student success and retention offices** that monitor risk trends and intervention impact.
- **Administrators** who care about retention, equity, and resource allocation.

## Proposed Solution & Architecture

The solution is a three-layer early warning platform:

- **Data Layer:** Synthetic but realistic higher-ed data stored in CSVs (and an optional SQLite database) capturing academics, engagement, finances, counseling, and ML-generated risk scores.
- **ML Layer:** A Random Forest model (plus supporting pipeline) that engineers ~69 features and outputs an `OverallRiskScore` and `RiskCategory` for each student.
- **UI Layer:** A Streamlit web application with role-based pages for students, advisors, and admins (dashboard, directory, analytics, student portal, admin portal, ML predictions, and interventions).

> **Architecture Diagram (for portfolio site):**
> Insert your system architecture image here (data sources → ML pipeline → Streamlit app → users).

## Data Design & ERD

The dataset is generated by a custom Python/Faker script that creates ~10,000 students and related tables such as enrollments, grades, attendance, logins, payments, counseling, and risk_scores. These tables are linked primarily by `StudentID` and `EnrollmentID`, forming a star-like schema that supports both ML feature engineering and dashboard queries.

> **ER Diagram (for portfolio site):**
> Insert your ER diagram image here showing core entities (Students, Enrollments, Courses, Grades, Attendance, Logins, Payments, Counseling, RiskScores, Terms, Departments, Faculty) and their relationships.

## Dashboard & Key Insights

The Streamlit app exposes several key views:

- **Advisor Dashboard:** KPI cards for total students, average GPA, and risk bands; a prioritized student list with filters by risk, classification, and first-generation status.
- **Students Directory:** A searchable table and 360° profile with Academic, Engagement, Financial, and Wellness tabs for each student.
- **Analytics:** Cohort-level charts (risk distribution, GPA histogram, risk by classification/first-gen/international) that reveal patterns such as elevated risk among freshmen and first-generation students.
- **Student Portal:** Personalized view of GPA, credits, courses, finances, counseling visits, and notifications.
- **Admin Portal & Interventions:** Executive KPIs, equity analysis, ML predictions tables, and an interventions log that links risk detection to concrete actions.

These insights show how risk is distributed, what is driving it, and which students and segments should be prioritized for outreach.

## Ethics, Limitations & Lessons Learned

Because the project uses synthetic data, no real students are affected; however, the design still follows principles that would matter in production: limiting access by role, avoiding stigmatizing language, and focusing on support rather than punishment. The main limitations are that the data and model are based on simulated patterns and mostly represent a single current term, so real institutional behavior may be more complex.

Through this work the team learned how difficult but valuable it is to connect data engineering, ML modeling, and user experience into one coherent system. Future improvements include deeper model explainability, real-time integration with live LMS/SIS systems, and formal evaluation of which interventions actually improve retention.
